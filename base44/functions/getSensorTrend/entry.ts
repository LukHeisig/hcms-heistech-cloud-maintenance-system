...
Deno.serve(async (req) => {
  try {
    ...
    // Čteme přímo uložené hodnoty z SensorData (has_fft záznamy mají vel_rms uložené při ingetu)
    ...
    // === TEPLOTA ===
    if (is_temperature) {
      const tempRecords = records.filter(r => r.temperature != null && r.temperature !== 28.0);
      if (trend_only) {
        const last10 = tempRecords.slice(-10);
        const temps = last10.map(r => r.temperature);
        return Response.json({
          trends: { temperature: temps.length >= 2 ? slopeToDirection(linearRegressionSlope(temps), temps, thresholdPct) : "stable" }
        });
      }
      return Response.json({
        data: tempRecords.map(r => ({
    ...
    // === VIBRACE — čteme přímo uložené RMS hodnoty z DB ===
    const data = records
      .map(r => ({
        ts: formatTs(r.created_date),
        sensor_data_id: r.id,
