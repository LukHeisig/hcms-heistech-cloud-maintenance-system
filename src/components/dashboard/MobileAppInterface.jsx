...
export default function MobileAppInterface({ user, isStandalone = false }) {
  ...
  const myPreventionMachines = useMemo(() => {
  ...
  // Need useEffect for search logic
  React.useEffect(() => {
    if (searchQuery.trim().length > 1) {
      const filtered = machines.filter(m => 
        m.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  return (
    <div className={`bg-slate-100 ${isStandalone ? 'min-h-screen pb-20' : 'h-full'}`}>
      {/* Mobile Header */}
      ...
      <div className="p-4 space-y-6">
        ...
        {/* Bottom Navigation Spacer for Standalone Mode */}
        {isStandalone && <div className="h-16"></div>}
      </div>

      {/* Bottom Sticky Nav for Standalone Mode */}
      {isStandalone && (
